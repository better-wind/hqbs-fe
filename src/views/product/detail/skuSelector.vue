...
<style scoped>
  .cont { width: 100%; max-width: 750px; background: #fff; position: fixed; bottom: 0; color: #000; z-index: 999; }
  .header { width: 100%; height: 97px; border-bottom: 1PX solid #eee; line-height: 54px; border-bottom: 1PX solid #d9d9d9; margin-left: 15px;}
  .header img { width: 97px; height: 97px; position: absolute; top: -10px; left: 15px; }
  .desc { margin-left: 105px; line-height: 1;  }
  .desc span { font-size: 22px; color: #ff0000; margin-top: 25px; }
  .desc em { width: 180px; font-size: 14px; color: #33383b; margin-top: 5px; }
  .close { display: inline-block; width: 54px; height: 54px; background: url('../../../assets/image/close.png') no-repeat center; background-size: 24%; top: 0; right: 0;}
  .selection { max-height: 300px; overflow-y: auto; }
  ul li { margin-left: 15px; border-bottom: 1PX solid #d9d9d9; }
  .prop-name { font-size: 14px; color: #858585; margin: 10px 0; }
  .number { display: flex; align-items: center; height: 65px; }
  .number .title { margin-left: 30px; color: #33383b; font-size: 14px; }
  .num-picker { right: 15px; }
  .submit { padding-left: 20px; padding-top: 20px; padding-bottom: 20px; border-top: 1PX solid #d9d9d9;}
  .submit p { width: 335px; height: 50px; line-height: 50px; text-align: center; font-size: 18px; color: #fff; background: #000; }
  .submit p i { display: inline-block; width: 18px; height: 18px; background: url('../../../assets/image/chance.png') no-repeat; background-size: 100%; vertical-align: middle; margin-right: 5px;}
  ...
  <transition name="slide">
    <div class="cont" v-if="showSku&&skuInfo">
      ...
      <div class="submit">
        ...
        <p v-else-if="product.status==1" @click="submitClick(1)">加入购物袋</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      close () {
        this.cb.closeSku(this.product.productId)
      },
      submitClick(type){
        this.cb.submitHandler(type,this.product.productId,this.buyCount )
      },
      ...
      init(){
        if(this.skuInfo){
          ...
          this.skuInfo.propList.map((propArr, pIndex) => {
            ...
            propArr.value.map((item,index) => {
              ...
              for (var i = 0; i < pList.length; i++) {
                let pItem = pList[i]
                if(pItem.stockCount>0 && pItem.propValueList.indexOf(item)>=0){
                  nItem.disabled = false
                  for(var j = 0; j < pItem.propValueList.length; j++){
                    if(j != pIndex) {
                      const prank = this.skuInfo.propList[j].value.indexOf(pItem.propValueList[j])
          ...
          for (var k = 0; k < pList.length; k++){
            if(pList[k].productId==this.skuInfo.defaultId){
