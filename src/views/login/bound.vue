...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('modLogin',['getAddress','getCode','boundMobile','setTid']),
      login(){
        let opts = {}
        opts.data = {mobileNumber:this.phoneNum,verificationCode:this.valiCode,addressCode:this.addrNo}
        this.boundMobile(opts).then(data => {
          if(data.status==0 && data.msg){ 
            this.showDlg(data.msg) 
          }else if(data.status==1){
            window.location.href = this.redirectUrl || 'home.html'
          }else if(data.status == 8){
            if(data.tid){
                this.setIntroducer(data.tid)
                return false
            }
            this.$router.push({path:window.routerPrefix + '/login/introducer',query:{redirect_url:this.redirectUrl}})
          }else if(data.status==0 && data.errorMessage){
      ...
      setIntroducer(tid){
        ...
        this.setTid(opts).then(data => {
          if(data.status==1){
            window.location.href = this.redirectUrl || '/hp/home.html'
          }else{
            this.$router.push({path:window.routerPrefix + '/login/introducer',query:{redirect_url:this.redirectUrl}})
          }
        })
      },
      getValiCode(type){//type 1 短信，2语音
        if(type==2 && !voiceDisable){ this.showDlg('请等候' + this.voiceCounter + 's后重试') }
        let opts = {}
        opts.data = {mobileNumber:this.phoneNum,addressCode:this.addrNo,channelType:type}
        this.getCode(opts).then(data => {
          if(data.status==0 && data.msg){ 
            ...
            if(type==2){
              ...
              this.timeCount(this.voiceCounter,(counter) => {//60秒内禁止发送语音
              ...
              this.timeCount(60,(counter) => {//发送短信倒计时
                this.codeTxt = counter + 's'
              },() => { this.codeTxt = '重发' })

              this.timeCount(15,(counter) => {//15秒后打开发送语音
              },() => { this.showVoice = true })
            }
          }
        })
