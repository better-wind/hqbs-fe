...
<template>
	<div>
	  ...
	  <div class="product-list">
	  	<div class="wrap">
	  		<div class="item" v-for="(item,index) in list" @click="handleLink(item.url)">
		  		<div>
...
export default {
  ...
  data() {
    return {
    	vipIcon: require('@/assets/image/vip-icon.png'),
    	isMember: false,
      list: [],
      state: null,
      showType: 0
  ...
  methods: { 
      ...
      this.state = $state
  		this.productListAction({params}).then(()=>{
  			this.isMember = this.sProductListObj.isMember == 1 ? true : false
